...
void Check_generation_time(unsigned char type, unsigned char interval_needed) {
    ...
    // 1. Seleccionar el tiempo de referencia seg�n el tipo
    switch(type) {
    ...
    // 2. C�lculo del tiempo transcurrido (Aritm�tica modular base 60)
    ...
    // 3. Si ha pasado el tiempo, ejecutamos la funci�n espec�fica y actualizamos
...
void ANIMALS_Motor(){
    ...
    switch(state){
        case 0:
            if(i < total_animals){
                Check_if_put_sleep(i); // hace falta a�adir los valores de input de la SIO
                i++;
            }else{
                state = 1;
                i = 0;
